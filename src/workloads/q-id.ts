...
export function qIdS1(_page: number): WorkloadFn {
  return async ({ tenantId, strategy }) => {
    ...
    if (!cursor) {
      ...
      // Use Number() for cursor values in query params — postgresjs types don't accept bigint
      const lastTs = Number(cursor.lastTs);
    ...
    if (rows.length > 0) {
      ...
      spansCursors.set(tenantId, {
        lastTs: BigInt(last.timestamp),
        lastId: String(last.span_id),
      });
    } else {
      // Reached end of data, reset cursor
...
export function qIdS2(_page: number): WorkloadFn {
  return async ({ tenantId, strategy }) => {
    ...
    if (!cursor) {
      ...
      // Use Number() for cursor values in query params
      const lastTs = Number(cursor.lastTs);
      const lastId = cursor.lastId;
